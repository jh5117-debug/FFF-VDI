...
FFF-VDI 使用受门控限制（Gated）的 Stable Video Diffusion 模型作为 img2vid 主干： stabilityai/stable-video-diffusion-img2vid-xt-1-1。

3.1 在浏览器中操作
打开 https://huggingface.co/stabilityai/stable-video-diffusion-img2vid-xt-1-1。

登录您的 Hugging Face 账号。

滚动到许可/条款部分，同意条款，填写简短表单（例如用途填写：“Academic non-commercial research / video inpainting”），然后提交。

前往 Settings (设置) → Access Tokens (访问令牌)，创建一个具有 Read (读取) 权限的新令牌（建议命名为 fff-vdi-h100）。
