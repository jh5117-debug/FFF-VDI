...
def main():
    ...
    # parameters_list += list(controlnet.parameters())
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                gt_flows_bi = fix_raft(pixel_values, iters=20)
                pred_flows_bi, _ = fix_flow_complete.forward_bidirect_flow(gt_flows_bi, mask_values)
                pred_flows_bi = fix_flow_complete.combine_flow(gt_flows_bi, pred_flows_bi, mask_values)

                conditional_pixel_values = masked_pixel_values
                ...
                target = latents
                model_pred = unet(inp_noisy_latents, mask_values, pred_flows_bi, timesteps, encoder_hidden_states, added_time_ids=added_time_ids).sample
                    ...
                    if False and ((global_step % args.validation_steps == 0) or (global_step == 1)):
                        ...
                        for num_demos in range(len(sub_folders)):
                            ...
                            for demo_path in demo_paths:
                                demo_img = load_image(demo_path).resize((args.width, args.height))
                            ...
                            for demo_mask_path in demo_mask_paths:
                                ...
                                square_mask = find_square_mask(demo_mask)
                                demo_masks.append(square_mask)
                            ...
                            with torch.autocast(str(accelerator.device).replace(":0", ""), enabled=accelerator.mixed_precision == "fp16"):
                                for val_img_idx in range(args.num_validation_images):
                                    ...
                                    for i in range(num_frames):
                                        ...
                                        concat_img = np.concatenate([gt, mask, pred, comp_img], axis=1)
